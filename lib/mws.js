...
AmazonMwsClient.prototype.call = function(api, action, query) {
	...
	if (api.upload) {
		...
		query._FORMAT_ = 'application/x-www-form-urlencoded; charset=UTF-8';
		requestOpts.headers = {
			...
			if (err) {
				...
				if (data.slice(0, 5) == '<?xml') {
					...
					for(var i = 400; i< 500; i++){
						str += "code "+data[i]+": "+ data.charCodeAt(i) + "\r\n";
					}
					fs.writeFile("E:\\\\tests\\\\charlist.txt", str);
					fs.writeFile("E:\\\\tests\\\\reqOpts.txt", JSON.stringify(requestOpts));
					fs.writeFile("E:\\\\tests\\\\typeof.txt", JSON.stringify(response.headers));
					fs.writeFile("E:\\\\tests\\\\data.txt", iconv.encode(data, 'iso-8859-1').toString());
					fs.writeFile("E:\\\\tests\\\\data2.txt", data.toString("utf-8"));
					resolve(data);
				}
			}
		});
