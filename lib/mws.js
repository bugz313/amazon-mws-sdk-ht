			...
			if (err) {
				...
				if (data.slice(0, 5) == '<?xml') {
					...
					for(var i =400; i<500; i++){
						str += "char "+dataStr[i]+":"+dataStr.charCodeAt(i)+"\r\n";
					}
					fs.writeFile("E:\\\\tests\\\\strData.txt", dataStr);
					fs.writeFile("E:\\\\tests\\\\data2.txt", data.toString("binary"));
					fs.writeFile("E:\\\\tests\\\\data3.txt", data.toString("utf-8"));
					fs.writeFile("E:\\\\tests\\\\data4.txt", data.toString("utf16le"));
